...
// --- Types mapping to your expected Golang Backend Structs ---
...
export default function MovementLogs() {
  ...
  const fetchLogsData = useCallback(async (page: number, currentFilter: string) => {
    ...
    try {
      /* 
        ========================================================
        🔥 UNCOMMENT THIS BLOCK TO USE YOUR REAL GOLANG BACKEND 🔥
        ========================================================
        
        const response = await fetch(`http://localhost:8080/api/v1/movement-logs?page=${page}&limit=${limit}&filter=${currentFilter}`);
        ...
        setStats(json.stats); // Assuming backend returns stats in the same call, or make a separate fetch
      */

      // --- 🧪 SIMULATED BACKEND DELAY FOR TESTING ---
  ...
  return (
    <div className="w-full min-h-screen bg-[#f8f9fc]">
      <main className="p-4 sm:p-6 lg:p-8 xl:p-10 max-w-[1600px] mx-auto">
        ...
        {/* --- Filters and Live Updates Indicator --- */}
        ...
        {/* --- Table Section --- */}
        <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden flex flex-col relative">
...
function StatCard({ value, label, icon, colors, isLoading }: StatCardProps) {
  return (
    <div className={`p-5 xl:p-8 rounded-[24px] xl:rounded-[32px] border ${colors.border} ${colors.bg} flex items-center gap-4 xl:gap-5 transition-all hover:shadow-sm`}>
      {/* Circle Icon Wrapper */}
...
const DUMMY_LOGS: MovementLog[] =[
  ...
  // Adding a few extra items to demonstrate actual pagination working
  { id: 7, time: "13:30:12", date: "OCT 24, 2023", deviceId: "CBE-LP-1122-Z", employee: "Abebe Kebede", gate: "Main North", action: "ENTRY" },
  { id: 8, time: "13:15:00", date: "OCT 24, 2023", deviceId: "CBE-LP-3344-Y", employee: "Chala Merera", gate: "East Deck", action: "EXIT" },
];